...
/**
 * The /picture diagnostic: survival status header, the keep-pace-with-inflation
...
export default function PesaDiagnostic({ answers }: { answers: JourneyAnswers }) {
  ...
  // After tax, and signed: the average deposit can keep pace, and the copy
  // must be able to say so rather than print a negative gap.
  const gapPoints = (CURRENT_INFLATION - BANK_NET_YIELD) * 100;
  ...
  return (
    <div className="print-grid-2 w-full max-w-2xl space-y-6">
      ...
      {/* ── Module 2: how savings keep pace with inflation ── */}
      {showBurner && (
        <section
          aria-label="How your savings keep pace"
          className="rounded-2xl bg-white p-5 shadow-sm"
        >
          <h2 className="text-base font-semibold text-primary">How your savings keep pace</h2>
          <div className="mt-4 space-y-3">
            <YieldBar
              label="Average bank deposit, after tax"
              ratePct={BANK_NET_YIELD * 100}
              barClass="bg-[#8B8578]"
            />
            <YieldBar
              label="Kenyan inflation"
              ratePct={CURRENT_INFLATION * 100}
              barClass="bg-danger"
            />
          </div>
          <p className="mt-4 rounded-xl bg-accent-soft p-3 text-sm text-ink-soft">
            💡 <em>The real picture:</em>{" "}
            {gapPoints > 0 ? (
              <>
                after tax, inflation is running{" "}
                <strong className="text-danger">{gapPoints.toFixed(2)} points ahead</strong> of the
                average bank deposit.
              </>
            ) : (
              <>after tax, the average bank deposit is roughly keeping pace with inflation.</>
            )}{" "}
            A regulated money market fund can add about{" "}
            <strong className="text-success">+{YIELD_UPSIDE_POINTS} points a year</strong> after
            tax — money that grows instead of standing still.
          </p>
        </section>
      )}
